# ruff: noqa: N999
"""Validate the Spores druid circle-spells feature through the character-growth consumer.

This feature is the first two-clause spell grant in the Tasha batch: a known
cantrip (chill touch) plus eight always-prepared circle spells.  The compiler
merges the two advancement blocks into one envelope and preserves each grant's
`grant_mode` inside `spell_grants`, so the materialized top-level `grant_mode`
is intentionally absent (the two clauses differ).  This script asserts the
consumer resolves all nine spells through the production character-growth path.
"""
...
RESULT_PATH = ROOT / "data/content-ir/compiled/production-runtime-results-XXXII.json"
REPORT_PATH = ROOT / "reports/tashas-feature-production-consumer-round-XXXI-2026-08-13.json"

FEATURE_CASES: tuple[tuple[str, dict[str, list[str]]], ...] = (
    ("content.tashas-cauldron.round2.feature.spores-druid-circle-spells", {}),
)

EXPECTED_SPELLS: tuple[str, ...] = (
    "chill_touch",
    "blindness_deafness",
    "gentle_repose",
    "animate_dead",
...
def _load_contracts() -> dict[str, tuple[FeatureSpec, dict[str, Any]]]:
    ...
    for path in sorted(FEATURE_ROOT.glob("*.json")):
        ...
        spells = advancement.get("spells")
        if not isinstance(spells, list) or tuple(spells) != EXPECTED_SPELLS:
            raise TypeError(
                f"selected feature must grant exactly {EXPECTED_SPELLS}: {spec.feature_id}"
            )
        grants = advancement.get("spell_grants") or []
        grant_modes = [item.get("grant_mode") for item in grants if isinstance(item, dict)]
        if grant_modes.count("known") != 1 or grant_modes.count("always_prepared") != 8:
            raise TypeError(
                "spores feature must preserve one known cantrip and eight always-prepared "
                f"grants, got {grant_modes}: {spec.feature_id}"
            )
        result[spec.feature_id] = (spec, contract)
    expected = {feature_id for feature_id, _ in FEATURE_CASES}
    if set(result) != expected:
        raise RuntimeError("Round XXXI selected feature contract is incomplete")
    return result
...
def _run_case(
    ...
    campaign = client.post(
        "/api/v1/campaigns", json={"name": "Tasha Round XXXI character growth"}
    ).json()
    ...
    character = client.post(
        ...
        json={
            ...
            "level": max(1, int(spec.level or 1)),
            "hp": 20,
            "max_hp": 20,
        },
    ).json()
    body = {
        ...
        "idempotency_key": f"tashas-round-XXXI-growth-{index:03d}",
    }
    ...
    evidence: dict[str, Any] = {
        ...
        "source": "round-II-reviewed-feature-runtime-through-round-XXXI-character-consumer",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as directory:
        database_url = f"sqlite:///{directory}/tashas-round-XXXI.db"
    ...
    checks = {
        ...
        "spores_grant_mode_split": all(
            item.get("spell_grant_count") == len(EXPECTED_SPELLS) for item in passed
        ),
        "formal_registry_written": False,
        "formal_database_written": False,
        "name_branch_count": 0,
        "protected_fingerprints_unchanged": protected == PROTECTED,
    }
    evidence_by_id = {item["content_id"]: item for item in results}
    result = {
        "schema_version": "content-ir-production-runtime-results-XXXII-1",
        "round_id": "round-XXXI",
        "content_kind": "advancement",
        ...
        "checks": checks,
        "source": (
            "Round-II reviewed Feature IR through real ContentIRRuntimeService character "
            "advancement on an isolated migrated database"
        ),
    }
    report = {
        "schema_version": "tashas-feature-production-consumer-round-XXXI-1",
        "round_id": "round-XXXI",
        "pack_id": "tashas-cauldron",
        "source_book": "塔莎的万事坩埚",
        "selected_feature_ids": [feature_id for feature_id, _ in FEATURE_CASES],
        "selected_clusters": [
            "advancement.fixed_spell_grant",
            "feature.clause.known_cantrip",
            "feature.clause.always_prepared",
            "content_ir_runtime.advancement_preview_confirm_replay",
        ],
        "selected_platforms": [
            "advancement_service.character_growth.v1",
            "advancement_service.spell_registry",
            "content_ir_runtime.advancement_character_cas",
        ],
        "results": results,
        "checks": checks,
        "formal_apply": False,
    ...
    if len(production_ids) != len(FEATURE_CASES):
        raise SystemExit("Round XXXI production consumer gate failed")
    return 0
