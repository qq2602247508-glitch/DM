# ruff: noqa: N999
"""Validate the existing generic passive/inspection Feature consumer for Tasha IR."""
...
RESULT_PATH = ROOT / "data/content-ir/compiled/production-runtime-results-VIII.json"
REPORT_PATH = ROOT / "reports/tashas-feature-production-consumer-round-VI-2026-08-12.json"

FEATURE_IDS = (
    "content.tashas-cauldron.round2.feature.alchemist-alchemical-savant",
    "content.tashas-cauldron.round2.feature.armorer-powered-steps",
    "content.tashas-cauldron.round2.feature.artificer-tool-expertise",
    "content.tashas-cauldron.round2.feature.artillerist-arcane-firearm",
    "content.tashas-cauldron.round2.feature.astral-self-armor-of-the-spirit",
    "content.tashas-cauldron.round2.feature.astral-self-astral-sight",
    "content.tashas-cauldron.round2.feature.feat-crusher",
    "content.tashas-cauldron.round2.feature.fey-wanderer-otherworldly-glamour",
)
...
def _load_contracts() -> dict[str, tuple[FeatureSpec, dict[str, Any]]]:
    ...
    if set(result) != set(FEATURE_IDS):
        raise RuntimeError("Round VI selected feature contract is incomplete")
    return result


def _registry(feature_id: str, spec: FeatureSpec, contract: dict[str, Any]) -> dict[str, Any]:
    grants = [
        {
...
def _run_feature(
    ...
    campaign = client.post(
        "/api/v1/campaigns", json={"name": "Tasha Round VI passive consumer"}
    ).json()
    base = f"/api/v1/campaigns/{campaign['id']}"
    character = client.post(
        f"{base}/characters",
        json={
            "name": "Tasha passive actor",
    ...
    combat = client.post(f"{base}/combats", json={"name": "Tasha passive combat"}).json()
    root = f"{base}/combats/{combat['id']}"
    registry = _registry(feature_id, spec, contract)
    actor = client.post(
        f"{root}/combatants",
        json={
            "display_name": "Tasha passive actor",
    ...
    body = {
        ...
        "idempotency_key": f"tashas-round-VI-passive-{index:03d}",
    }
    ...
    evidence: dict[str, Any] = {
        ...
        "source": "round-II-reviewed-feature-contract-through-round-VI-passive-consumer",
    ...
    evidence.update(
        {
            ...
            "inspection_resolution": action.get("resolution_kind") == "inspection",
        }
    )
    if confirmed.status_code != 200:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as directory:
        database_url = f"sqlite:///{directory}/tashas-round-VI.db"
    ...
    passed = [
        ...
        and item.get("passive_registry_bound")
        and item.get("inspection_resolution")
    ]
    ...
    checks = {
        ...
        "all_passive_registry_bound": all(item.get("passive_registry_bound") for item in passed),
        "all_inspection_resolution": all(item.get("inspection_resolution") for item in passed),
    ...
    _write(
        RESULT_PATH,
        {
            "schema_version": "content-ir-production-runtime-results-VIII-1",
            "production_runtime_full_ids": production_ids,
            "evidence_by_id": evidence_by_id,
            "checks": checks,
            "source": "Round-II reviewed Feature IR through real ContentIRRuntimeService passive inspection on an isolated migrated database",
        },
    )
    _write(
        REPORT_PATH,
        {
            "schema_version": "tashas-feature-production-consumer-round-VI-1",
            "pack_id": "tashas-cauldron",
            "source_book": "塔莎的万事坩埚",
    ...
    if len(production_ids) != len(FEATURE_IDS):
        raise SystemExit("Round VI production consumer gate failed")
    return 0
