# ruff: noqa: N999
"""Validate three cleric Domain Spells features through the character-growth consumer."""
...
RESULT_PATH = ROOT / "data/content-ir/compiled/production-runtime-results-XXIX.json"
REPORT_PATH = ROOT / "reports/tashas-feature-production-consumer-round-XXVIII-2026-08-13.json"

FEATURE_CASES: tuple[tuple[str, dict[str, list[str]]], ...] = (
    ("content.tashas-cauldron.round2.feature.order-cleric-domain-spells", {}),
    ("content.tashas-cauldron.round2.feature.peace-cleric-domain-spells", {}),
    ("content.tashas-cauldron.round2.feature.twilight-cleric-domain-spells", {}),
)
...
def _load_contracts() -> dict[str, tuple[FeatureSpec, dict[str, Any]]]:
    ...
    for path in sorted(FEATURE_ROOT.glob("*.json")):
        ...
        if not isinstance(spells, list) or len(spells) != 10:
            raise TypeError(f"selected feature must grant exactly 10 spells: {spec.feature_id}")
        result[spec.feature_id] = (spec, contract)
    expected = {feature_id for feature_id, _ in FEATURE_CASES}
    if set(result) != expected:
        raise RuntimeError("Round XXVIII selected feature contract is incomplete")
    return result
...
def _run_case(
    ...
    campaign = client.post(
        "/api/v1/campaigns", json={"name": "Tasha Round XXVIII character growth"}
    ).json()
    ...
    body = {
        ...
        "idempotency_key": f"tashas-round-XXVIII-growth-{index:03d}",
    }
    ...
    evidence: dict[str, Any] = {
        ...
        "source": "round-II-reviewed-feature-runtime-through-round-XXVIII-character-consumer",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as directory:
        database_url = f"sqlite:///{directory}/tashas-round-XXVIII.db"
    ...
    result = {
        "schema_version": "content-ir-production-runtime-results-XXIX-1",
        "round_id": "round-XXVIII",
        "content_kind": "advancement",
    ...
    report = {
        "schema_version": "tashas-feature-production-consumer-round-XXVIII-1",
        "round_id": "round-XXVIII",
        "pack_id": "tashas-cauldron",
        "source_book": "塔莎的万事坩埚",
    ...
    if len(production_ids) != len(FEATURE_CASES):
        raise SystemExit("Round XXVIII production consumer gate failed")
    return 0
