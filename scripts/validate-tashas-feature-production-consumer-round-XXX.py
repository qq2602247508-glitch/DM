# ruff: noqa: N999
"""Validate three artificer spell-list features through the character-growth consumer."""
...
RESULT_PATH = ROOT / "data/content-ir/compiled/production-runtime-results-XXXI.json"
REPORT_PATH = ROOT / "reports/tashas-feature-production-consumer-round-XXX-2026-08-13.json"

FEATURE_CASES: tuple[tuple[str, dict[str, list[str]]], ...] = (
    ("content.tashas-cauldron.round2.feature.battle-smith-spell-list", {}),
    ("content.tashas-cauldron.round2.feature.armorer-spell-list", {}),
    ("content.tashas-cauldron.round2.feature.artillerist-spell-list", {}),
)
...
def _load_contracts() -> dict[str, tuple[FeatureSpec, dict[str, Any]]]:
    ...
    if set(result) != expected:
        raise RuntimeError("Round XXX selected feature contract is incomplete")
    return result
...
def _run_case(
    ...
    campaign = client.post(
        "/api/v1/campaigns", json={"name": "Tasha Round XXX character growth"}
    ).json()
    ...
    body = {
        ...
        "idempotency_key": f"tashas-round-XXX-growth-{index:03d}",
    }
    ...
    evidence: dict[str, Any] = {
        ...
        "source": "round-II-reviewed-feature-runtime-through-round-XXX-character-consumer",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as directory:
        database_url = f"sqlite:///{directory}/tashas-round-XXX.db"
    ...
    result = {
        "schema_version": "content-ir-production-runtime-results-XXXI-1",
        "round_id": "round-XXX",
        "content_kind": "advancement",
    ...
    report = {
        "schema_version": "tashas-feature-production-consumer-round-XXX-1",
        "round_id": "round-XXX",
        "pack_id": "tashas-cauldron",
        "source_book": "塔莎的万事坩埚",
    ...
    if len(production_ids) != len(FEATURE_CASES):
        raise SystemExit("Round XXX production consumer gate failed")
    return 0
