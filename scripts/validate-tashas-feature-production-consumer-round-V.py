...
RESULT_PATH = ROOT / "data/content-ir/compiled/production-runtime-results-VII.json"
REPORT_PATH = ROOT / "reports/tashas-feature-production-consumer-round-V-2026-08-12.json"
...
FEATURE_CASES: tuple[tuple[str, dict[str, list[str]]], ...] = (
    ...
    ("content.tashas-cauldron.round2.feature.peace-cleric-implement-of-peace", {}),
    ("content.tashas-cauldron.round2.feature.rune-knight-bonus-proficiencies", {}),
    ("content.tashas-cauldron.round2.feature.twilight-cleric-bonus-proficiencies", {}),
    (
        "content.tashas-cauldron.round2.feature.order-cleric-bonus-proficiencies",
        {"intimidation_or_persuasion": ["persuasion"]},
    ),
    (
        "content.tashas-cauldron.round2.feature.feat-skill-expert",
        {"chosen_skill": ["stealth"]},
    ),
    (
        "content.tashas-cauldron.round2.feature.ranger-canny",
        {"chosen_skill": ["survival"], "chosen_language": ["Elvish"]},
    ),
    ("content.tashas-cauldron.round2.feature.aberrant-mind-psionic-spell-list", {}),
)
...
def _contracts() -> dict[str, dict[str, Any]]:
    ...
    contracts = payload.get("runtime_contracts")
    if not isinstance(contracts, dict):
        raise RuntimeError("feature runtime registry lacks runtime_contracts")
    result = {feature_id: dict(contracts[feature_id]) for feature_id, _ in FEATURE_CASES}
    if len(result) != len(FEATURE_CASES):
        raise RuntimeError("Round V feature case list contains duplicate ids")
    return result
...
def _run_case(
    ...
    campaign = client.post(
        "/api/v1/campaigns", json={"name": "Tasha Round V character growth"}
    ).json()
    ...
    character = client.post(
        ...
        json={
            ...
            "class_name": str(contract.get("class_name") or "玩家角色"),
            "level": max(1, int(contract.get("class_level") or 3)),
            "hp": 20,
            "max_hp": 20,
        },
    ).json()
    body = {
        ...
        "idempotency_key": f"tashas-round-V-growth-{index:03d}",
    }
    ...
    evidence: dict[str, Any] = {
        ...
        "source": "round-II-reviewed-feature-runtime-registry-through-round-V-character-consumer",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as directory:
        database_url = f"sqlite:///{directory}/tashas-round-V.db"
        ...
        with TestClient(app) as client:
            for index, (feature_id, choices) in enumerate(FEATURE_CASES, start=1):
                results.append(
                    _run_case(client, feature_id, contracts[feature_id], choices, index)
                )
    ...
    _write(
        RESULT_PATH,
        {
            "schema_version": "content-ir-production-runtime-results-VII-1",
    ...
    _write(
        REPORT_PATH,
        {
            "schema_version": "tashas-feature-production-consumer-round-V-1",
            "pack_id": "tashas-cauldron",
            "source_book": "塔莎的万事坩埚",
    ...
    if len(production_ids) != len(FEATURE_CASES):
        raise SystemExit("Round V production consumer gate failed")
    return 0
