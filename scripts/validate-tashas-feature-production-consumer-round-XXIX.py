# ruff: noqa: N999
"""Validate three always-prepared spell-list features through the character-growth consumer."""
...
RESULT_PATH = ROOT / "data/content-ir/compiled/production-runtime-results-XXX.json"
REPORT_PATH = ROOT / "reports/tashas-feature-production-consumer-round-XXIX-2026-08-13.json"

FEATURE_CASES: tuple[tuple[str, dict[str, list[str]]], ...] = (
    ("content.tashas-cauldron.round2.feature.wildfire-druid-circle-spells", {}),
    ("content.tashas-cauldron.round2.feature.watchers-paladin-oath-spells", {}),
    ("content.tashas-cauldron.round2.feature.glory-paladin-oath-spells", {}),
)
...
def _load_contracts() -> dict[str, tuple[FeatureSpec, dict[str, Any]]]:
    ...
    if set(result) != expected:
        raise RuntimeError("Round XXIX selected feature contract is incomplete")
    return result
...
def _run_case(
    ...
    campaign = client.post(
        "/api/v1/campaigns", json={"name": "Tasha Round XXIX character growth"}
    ).json()
    ...
    body = {
        ...
        "idempotency_key": f"tashas-round-XXIX-growth-{index:03d}",
    }
    ...
    evidence: dict[str, Any] = {
        ...
        "source": "round-II-reviewed-feature-runtime-through-round-XXIX-character-consumer",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as directory:
        database_url = f"sqlite:///{directory}/tashas-round-XXIX.db"
    ...
    result = {
        "schema_version": "content-ir-production-runtime-results-XXX-1",
        "round_id": "round-XXIX",
        "content_kind": "advancement",
    ...
    report = {
        "schema_version": "tashas-feature-production-consumer-round-XXIX-1",
        "round_id": "round-XXIX",
        "pack_id": "tashas-cauldron",
        "source_book": "塔莎的万事坩埚",
    ...
    if len(production_ids) != len(FEATURE_CASES):
        raise SystemExit("Round XXIX production consumer gate failed")
    return 0
