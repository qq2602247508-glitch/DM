...
if [ "$(wc -l <"$state_dir/cache.log" | tr -d ' ')" -ne 3 ]; then
...
if ! grep -F "python|$expected|-m alembic -c backend/alembic.ini upgrade head" "$state_dir/cache.log" >/dev/null; then
