...
## 2026-08-08 长执行检查点（自然守御）

- 固定分母 499；当前真实状态：`full 240 / partial 183 / dm_only 76`，本 Goal 从 `227/195/77` 起已真实净增 13。
- 新增德鲁伊大地结社「自然守御」：长休由玩家/DM 选择荒漠、极地、温带或热带并持久化；伤害防御按该选择映射火焰/寒冷/闪电/毒素抗性，条件免疫解析器独立消费中毒免疫，缺失/非法选择不授予抗性。
- 真实提交：`0510698 feat: automate nature ward terrain defense`；代码与文档提交分离，两个规定未跟踪路径保持不变。

## 2026-08-08 长执行检查点（战斗激励）

- 固定分母 499；当前真实状态：`full 241 / partial 182 / dm_only 76`，本 Goal 从 `227/195/77` 起已真实净增 14。
- 新增吟游诗人勇气学院「战斗激励」：真实激励骰记录携带防御/进攻模式；玩家提交模式和骰值后，攻击服务分别执行 AC 防御或命中后首段伤害加值，并原子消费目标/攻击者持有的骰子，重放不重复结算。
- 代码提交：`b5609f3 feat: automate combat inspiration modes`；两个规定未跟踪路径仍未暂存或提交。
