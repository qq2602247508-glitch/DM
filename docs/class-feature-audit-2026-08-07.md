...
- 固定分母 499；当前真实状态：`full 229 / partial 193 / dm_only 77`。
- 本长执行从 `full 227 / partial 195 / dm_only 77` 开始，已闭环预言师「预兆」与「高等预兆」：长休提交真实 1–20 骰值，玩家在检定前选择单颗骰子替换，真实检定入口消费并通过资源 CAS 与 action 请求幂等重放保护。
- 预兆骰池不由服务器伪造结果；长休必须由玩家/DM 提交完整骰池，逐颗骰子只能消费一次；未完成提交时长休会清空旧骰池。
- 仍未把只存在领域 helper、只落库或只覆盖单一分支的 `roll_intervention` 候选计入 full。
- 预兆相关定向测试、Ruff、compileall、`git diff --check` 已通过；全量 pytest 将作为本检查点门禁继续运行。
...
- 运行时状态：`{'full': 229, 'dm_only': 77, 'partial': 193}`
