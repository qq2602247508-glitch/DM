...
- 固定分母 499；当前真实状态：`full 227 / partial 195 / dm_only 77`。
- 本长执行从 `full 201 / partial 221 / dm_only 77` 开始，上一完整批次净增 `full +25`；随后下一批已安全闭环“狂怒”`+1`，尚未把该批包装成完成批次。
- 本检查点真实接入：自然恢复、百折不挠、妖冶娴都、狂怒。涉及短休/长休恢复、死亡豁免双骰与阈值、血量回合触发、魅力检定修正、技能选择持久化、狂暴/鲁莽首击附伤骰与每回合幂等。
- `roll_intervention` 的优势/劣势抵消仍只有领域解析器测试，未计入 full；未接入真实生产消费者的 helper 不计数。
- 全量 pytest、Ruff、compileall、`git diff --check` 均通过；仅有既有 Starlette/httpx 弃用警告。
...
- 运行时状态：`{'full': 227, 'dm_only': 77, 'partial': 195}`
