...
- 法术 slug 逐一对照 `玩家手册 2014` spell corpus 英文别名（`burning_hands`、`cure_wounds`、`flaming_sphere`、`scorching_ray`、`plant_growth`、`revivify`、`aura_of_life`、`fire_shield`、`flame_strike`、`mass_cure_wounds` / `alarm`、`detect_magic`、`moonbeam`、`see_invisibility`、`counterspell`、`nondetection`、`aura_of_purity`、`banishment`、`hold_monster`、`scrying` / `guiding_bolt`、`heroism`、`enhance_ability`、`magic_weapon`、`haste`、`protection_from_energy`、`compulsion`、`freedom_of_movement`、`commune`、`flame_strike`），三张法术表互不重叠。
...
- `ContentIRRuntimeService` 新增 `resolution_kind=communication` 只读分支：真实检查 actor 与 target 均 `submerged`，任一不满足 fail closed（400）；preview→confirm→replay、stale actor 409 CAS、OperationTransaction applied 全通过；顺带修复非 teleport 分支未初始化 `teleport_preview` 的 `UnboundLocalError`。
...
- Round XXVI validator 13/13 checks true；Round XXV reconciliation validator 18/18 checks true；focused Round XXV/XXVI receipt suite 8 passed；backend 全量 pytest 926 passed（仅既有 Starlette/httpx deprecation warning）；Ruff、compileall、diff-check 通过；whole-pack migration 连续两次 stdout SHA-256 `e87a73fac5289a265cf7a5b780daa5546d20ce739ab52e9bf7d313f1eb5c8fbe`。
- Round XXVI 分离提交 `feb5fb7`（generic roll intervention consumer）、`d4fa353`（Ambush dual-clause authored IR）、`3b08410`（test + validator + evidence）、`bb61ae1`（docs/ledger closeout）、`74bb1d2`（commit 集合记录）已推送到 `origin/main`（`a7f8586..74bb1d2`，2026-08-13）。push receipt 已写入 ledger。
