...
- 代码提交 `c03f8c9`：坚韧狂暴积木从 `partial` 提升为 `full`，接入普通伤害、怪物区域伤害逐目标结算和持续伤害 tick 的统一 0 HP 处理链。目标必须是权威角色、快照含 `relentless_rage:zero_hit_points_save`、当前处于 `raging`、野蛮人等级可从角色/运行时等级读取，且剩余伤害未达到最大生命值；缺任何条件都 fail-closed。
...
- 当前切片完成度：坚韧狂暴 0 HP 豁免、恢复、递增 DC、直接死亡例外、短休/长休重置 `100%`。它属于既有固定大项“职业/子职业 1–20 级运行时闭环”，没有新增第五类缺口；高级三维战斗仍按用户要求跳过。
