...
function ScenesContent({ campaignId }: { campaignId: string }): ReactElement {
  ...
  const [travelEncounterTitle, setTravelEncounterTitle] = useState("");
  const [travelEncounterSummary, setTravelEncounterSummary] = useState("");
  const [travelEncounterOutcome, setTravelEncounterOutcome] = useState<"avoided" | "resolved" | "evaded">("resolved");
  ...
  const travelConfirm = useMutation({
    ...
    onSuccess: () => {
      setTravelDraft(null);
      setTravelEncounterTitle("");
      setTravelEncounterSummary("");
    ...
    <div className="mx-auto max-w-[1200px] p-4 lg:p-6">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[0.85fr_1.15fr]" id="scene-editor">
        <Panel eyebrow="地点的当前可玩快照" title="场景">
          ...
          <div className="mt-4 rounded border border-ink-700 p-3"><p className="m-0 text-xs font-medium text-parchment-100">地点旅行</p><div className="mt-2 flex flex-wrap gap-2"><select className={selectCls} onChange={(event) => setTravelLocationId(event.target.value)} value={travelLocationId}><option value="">目的地</option>{locations.data?.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select><input className={inputCls} min="0" onChange={(event) => setTravelDistance(event.target.value)} type="number" value={travelDistance} /><select aria-label="旅行遭遇结果" className={selectCls} onChange={(event) => setTravelEncounterOutcome(event.target.value as typeof travelEncounterOutcome)} value={travelEncounterOutcome}><option value="resolved">遭遇已解决</option><option value="avoided">遭遇已避开</option><option value="evaded">遭遇已逃脱</option></select><Button disabled={!travelLocationId || Boolean(travelEncounterTitle.trim()) !== Boolean(travelEncounterSummary.trim())} onClick={() => travelPreview.mutate({ to_location_id: travelLocationId, distance_miles: Number(travelDistance), pace: "normal", encounter: travelEncounterTitle.trim() && travelEncounterSummary.trim() ? { title: travelEncounterTitle.trim(), summary: travelEncounterSummary.trim(), outcome: travelEncounterOutcome, visibility: "players" } : undefined })}>预览旅行</Button></div><div className="mt-2 grid gap-2 sm:grid-cols-2"><input className={inputCls} onChange={(event) => setTravelEncounterTitle(event.target.value)} placeholder="可选：旅行遭遇标题" value={travelEncounterTitle} /><textarea className={`${inputCls} min-h-10`} onChange={(event) => setTravelEncounterSummary(event.target.value)} placeholder="可选：遭遇结果摘要（确认后写入事件）" value={travelEncounterSummary} /></div>{travelDraft ? <div className="mt-2 text-xs text-stone-400">预计 {String(travelDraft.preview.duration_minutes)} 分钟{travelDraft.input.encounter ? ` · 已记录遭遇：${travelDraft.input.encounter.title}` : ""} <Button className="ml-2" loading={travelConfirm.isPending} onClick={() => travelConfirm.mutate()} size="sm" variant="primary">DM 确认</Button></div> : null}</div>
        </Panel>
