  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <section className={cardCls}>
        ...
        <div className="grid grid-cols-3 gap-2">{[["HP", `${character.hp}/${character.max_hp}`], ["AC", character.armor_class], ["速度", `${character.speed}尺`]].map(([label, value]) => <div className="rounded bg-ink-950 p-3 text-center" key={label}><span className="block text-2xs text-stone-500">{label}</span><strong className="font-mono text-lg">{value}</strong></div>)}</div>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      ...
      <aside className="space-y-4">
        <section className={cardCls}>
          ...
          <label className="block text-xs text-stone-400">攻击/技能<select className={`${inputCls} mt-1`} disabled={!combat.is_my_turn} onChange={(event) => setActionName(event.target.value)} value={actionName}><option value="">选择角色卡动作</option>{actions.map((action) => <option key={display(action.name)} value={display(action.name)}>{display(action.name)} · {display(action.damage ?? action.description ?? "")}</option>)}</select></label>
          <label className="mt-2 block text-xs text-stone-400">目标<select className={`${inputCls} mt-1`} disabled={!combat.is_my_turn} onChange={(event) => setTargetId(event.target.value)} value={targetId}><option value="">选择合法敌人</option>{enemies.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.health_status}</option>)}</select></label>
          ...
          <Button className="mt-3 w-full" disabled={!combat.is_my_turn || !actionName || !targetId || !attackTotal || !damageTotal || !own?.action_available} loading={mutation.isPending} onClick={() => mutation.mutate(() => attackWithMyCombatant(targetId, actionName, Number(attackTotal), Number(damageTotal)))} variant="primary">提交攻击并同步结算</Button>
          <Button className="mt-2 w-full" disabled={!combat.is_my_turn} onClick={() => mutation.mutate(() => endMyTurn(combat.version))}>结束我的回合</Button>
