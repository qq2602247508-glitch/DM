...
function CombatView({ snapshot, refresh }: { snapshot: PlayerRoomSnapshot; refresh: () => void }): ReactElement {
  ...
  const selectedFeatureId = typeof selectedActionBase?.feature_id === "string"
    ...
    >
      ...
      <aside className="min-w-0 space-y-4" data-testid="player-combat-sidebar">
        <section className={cardCls}>
          ...
          <Button className="mt-3 w-full" disabled={Boolean(selectedSummonBlock) || ended || !combat.is_my_turn || !actionName || !targetId || (isSelfRestoration && !conditionToRemove) || (!isAutoHitAction && !isCastAction && !isRuntimeFeatureAction && !isSavingThrowAction && !attackTotal) || (needsDamageRoll && usesLegacyDamageTotal && !damageTotal) || (featureNeedsHealingRoll && !damageTotal) || !divineSmiteDamageValid || !componentInputValid || !specialInputValid || !selectedActionAvailable || !featureActionValid} loading={mutation.isPending} onClick={() => mutation.mutate(async () => {
            if (isRuntimeFeatureAction) {
              const featureId = typeof selectedAction?.feature_id === "string" ? selectedAction.feature_id : "";
              const result = await submitMyFeatureAction(featureId, targetId, featureNeedsHealingRoll ? Number(damageTotal) : null, isCunningAction ? cunningActionChoice : undefined, isCunningAction && featureActionOutcome ? featureActionOutcome : undefined, isCunningAction ? featureActionNote.trim() || undefined : undefined, conditionToCure || undefined, conditionToRemove || undefined) as { result?: { feature_name?: string; healing?: { hp_gained?: number; }; condition_cure?: { condition?: string }; condition_removal?: { condition?: string }; selected_action?: string; } };
              setAttackTotal("");
              setDamageTotal("");
