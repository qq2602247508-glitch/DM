...
import { confirmNarrativeTransaction, createNarrative, listCharacters, listNarrative, listNpcs, previewNarrativeTransaction, updateNarrative } from "../api/entities";
...
function NarrativeContent({ campaignId }: { campaignId: string }): ReactElement {
  ...
  return <div className="mx-auto max-w-[1200px] p-4 lg:p-6"><div className="mb-4 flex flex-wrap gap-1.5">{tabs.map((item) => <button className={`rounded-md border px-3 py-1.5 text-sm ${tab === item.id ? "border-ember-500/60 bg-ember-500/10 text-ember-200" : "border-ink-600 text-stone-500"}`} key={item.id} onClick={() => setTab(item.id)} type="button">{item.label}</button>)}</div>
    ...
    <Panel className="mt-4" eyebrow="叙事运行器 · 先预览，后确认" title="非战斗遭遇与奖励"><div className="flex flex-wrap gap-2"><Button onClick={() => previewMutation.mutate({ idempotency_key: createClientId("narrative"), operations: [{ kind: "runtime", mode: "skill_challenge", title: "技能挑战", successes: 0, failures: 0 }] })} size="sm">技能挑战</Button>{(["chase", "negotiation", "stealth", "investigation"] as const).map((mode) => <Button key={mode} onClick={() => previewMutation.mutate({ idempotency_key: createClientId("narrative"), operations: [{ kind: "runtime", mode, title: mode, successes: 0, failures: 0 }] })} size="sm" variant="ghost">{({ chase: "追逐", negotiation: "谈判", stealth: "潜行", investigation: "调查" })[mode]}</Button>)}</div>{preview ? <div className="mt-3 rounded border border-amber-500/30 bg-ink-800 p-3 text-xs text-stone-300"><p className="m-0">已预览 {preview.input.operations.length} 个变更；不会在此之前写入。</p><Button className="mt-2" loading={confirmMutation.isPending} onClick={() => confirmMutation.mutate()} size="sm" variant="primary">DM 确认写入</Button></div> : null}</Panel>
