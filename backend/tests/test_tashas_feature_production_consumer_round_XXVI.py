...
def test_round_xxvi_receipt_matches_current_pack_projection() -> None:
    ...
    assert migration["production_full"] == 89
    assert migration["dm_assisted"] == 2
    assert migration["game_usable"] == 91
    assert migration["compile_only"] == 3
    assert migration["content_id_funnel"]["relation_holds"] is True
    assert len(load_production_runtime_evidence(ROOT, pack_id=PACK_ID)) == 132
    assert len(existing_project_production_ids(ROOT)) == 189
