"""add spells equipment attunement and economy tables

Revision ID: b41c9e7a2d30
...
def upgrade() -> None:
    op.create_table(
        ...
        *_stamp(),
        ...
        sa.CheckConstraint("spell_level >= 0 AND spell_level <= 9", name="ck_known_spell_level"),
        sa.UniqueConstraint("character_id", "name", name="uq_known_spell_character_name"),
    )
    op.create_index(
        "ix_known_spells_character", "known_spells", ["character_id", "created_at", "id"]
    )
    op.create_table(
        "prepared_spells",
        ...
        *_stamp(),
        sa.ForeignKeyConstraint(["known_spell_id"], ["known_spells.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["character_id"], ["characters.id"], ondelete="CASCADE"),
        sa.UniqueConstraint(
    ...
    op.create_table(
        ...
        sa.Column("attunement_required", sa.Boolean(), server_default="0", nullable=False),
    ...
    op.create_table(
        ...
        *_stamp(),
        ...
        sa.ForeignKeyConstraint(
            ["equipment_instance_id"], ["equipment_instances.id"], ondelete="CASCADE"
        ),
        sa.CheckConstraint("status IN ('active','ended')", name="ck_attunement_status"),
        sa.UniqueConstraint("equipment_instance_id", name="uq_attunement_equipment"),
    )
    op.create_index(
        "ix_attunements_character_status", "attunements", ["character_id", "status", "id"]
    )
    op.create_table(
        "wallets",
        sa.Column("campaign_id", sa.String(36), nullable=False),
        ...
        *_stamp(),
        ...
        sa.UniqueConstraint("campaign_id", "character_id", name="uq_wallet_campaign_character"),
    )
    op.create_table(
    ...
    op.create_table(
        ...
        *_stamp(),
        ...
        sa.CheckConstraint("quantity >= 0 AND price_copper >= 0", name="ck_shop_inventory_bounds"),
    )
    op.create_index(
        "ix_shop_inventory_campaign", "shop_inventories", ["campaign_id", "created_at", "id"]
    )


def downgrade() -> None:
    for table, indexes in (
        ("shop_inventories", ["ix_shop_inventory_campaign"]),
        ("currency_transactions", ["ix_currency_transactions_wallet"]),
        ("attunements", ["ix_attunements_character_status"]),
        ("equipment_instances", ["ix_equipment_campaign_character"]),
        ("known_spells", ["ix_known_spells_character"]),
    ):
        for index in indexes:
            op.drop_index(index, table_name=table)
        op.drop_table(table)
    op.drop_table("wallets")
    op.drop_table("prepared_spells")
